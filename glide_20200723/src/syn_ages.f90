...
! Syn_ages calculates synthetic ages by solving the heat equation in 1D
! Material paths are tracked through the evolving thermal field and used to 
! predict ages
...
real Tc
real dz,xint
real tstart,tend,dt,xtime,misfit
real sample_age,cooling,frac,dist
real,dimension(:,:),allocatable::temp,temperature,age,diag,sup,inf,f,deep
real,dimension(:),allocatable::exhum,geotherm,ax,bx,cx,lambda,alpha,query,b_flux

allocate(geotherm(params%n))

!$omp parallel private(i,j,k,nt,id,first,last,chunk,p,nproc,temp,exhum,xtime,iflag,Tc,sample_age,xint,temperature, &
!$omp& age,diag,sup,inf,f,alpha,lambda,ax,bx,cx,query,pos,ntsteps,b_flux,deep,frac,dist) shared(geotherm,tstart,tend,dt,mz)
...
!  print*,"here!",matrices%zz(i),j,dist
 !   j=j=params%m_max-floor(matrices%ta(i)/params%deltat) 
  ...
  enddo
  j=j+1
!  if (j.le.params%m_max+1-j) !print*, matrices%zz(i)
  dist = dist - matrices%tsteps(params%m_max+1-j)*matrices%edot_dat(j+(i-1)*params%m_max)
  matrices%syn_age(i) = matrices%syn_age(i) - matrices%tsteps(params%m_max+1-j)
  frac = (matrices%zz(i) - dist)/matrices%edot_dat(j+(i-1)*params%m_max)
print*,matrices%syn_age(i),frac
  matrices%syn_age(i) = matrices%syn_age(i) + frac
enddo
pause

print*,'min zc=',minval(matrices%zc),'max zc=',maxval(matrices%zc)
print*,"min geo=",minval(geotherm),"max geo=",maxval(geotherm)
