...
enddo 
! write heat flux out
        write(124,*) matrices%x_true(kk),matrices%y_true(kk),2.3*(temp(2)-temp(1))/dz
...
do i=1,mz-1
  ...
  if (temp_age(i).gt.closure(i)) then
     ...
     xjunk=M*(Tc-closure(i))
      ...
      if (matrices%isys(kk).gt.0) matrices%zc(kk)=dz*float(i-1)+xjunk
    exit
   endif
enddo
