...
integer id,nproc,pos,ntsteps,kk
...
!shared(geotherm,tstart,tend,dt,mz,tempfull)
...
!$omp end parallel
print*,'writing heat flux file'
open(124,file=params%run//"/stuff/heat_flux.txt",status="unknown")
do kk=1,params%n
        write(124,*) matrices%x_true(kk),matrices%y_true(kk),2.6*geotherm(kk)
enddo
close(124)
print*,'heat flux file written'
...
do i=1,params%n
  ...
  do while (dist.lt.matrices%zz(i))
    ...
    j=j-1
  enddo
 ! print*,"done",i,dist,matrices%zz(i),params%m_max+1-j
  j=j+1
!  if (j.le.params%m_max+1-j) print*, "this shouldn't be",matrices%zz(i),dist!,j,params%m_max+1-j
  dist = dist - matrices%tsteps(params%m_max+1-j)*matrices%edot_dat(j+(i-1)*params%m_max)
  matrices%syn_age(i) = matrices%syn_age(i) - matrices%tsteps(params%m_max+1-j)
  frac = (matrices%zz(i) - dist)/matrices%edot_dat(j+(i-1)*params%m_max)
!print*,matrices%syn_age(i),matrices%ta(i),frac,dist
!pause
  matrices%syn_age(i) = matrices%syn_age(i) + frac
enddo

print*,'min zc=',minval(matrices%zz),'max zc=',maxval(matrices%zz)
